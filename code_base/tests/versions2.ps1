...
#   Requirements query.
#
$fulluri = Add_route -uri $myuri -routeId 97
...
if ($Response.StatusCode -eq 200) {
   ...
   if ($mine.result.EsbApiVersion.Contains("25")) {
      Write-Host "ASB - /Versions on" $which":  Test Passed " -f Green
      exit 0
   }
}
...
Write-Host "ASB - /Versions on" $which":  Test Failed " -f Red
exit 1
