...
#
#
$argSet = addArguments -newArgName "StartDate" -newArgValue "2025-01-01"
$argSet = addArguments -sofar $argSet -newArgName "EndDate" -newArgValue "2025-06-01"
#$fulluri = Add_route -uri $myuri -route "Requirements/SubmissionStatuses/ProgramOfficeUserChanges" -query $argSet
$fulluri = Add_route -uri $myuri -routeId 81 -query $argSet
...
if ($Response.StatusCode -eq 200) {
   ...
   if ($mine.Result.Count -gt 0) {
      if ($mine.Result[0].EmailList[0].Contains("$sv")) {
         Write-Host "ASB - /Requirements/SubmissionStatuses/ProgramOfficeUserChanges on" $which":  Test Passed " -f Green
         exit 0
      }
   }
}
...
#Write-Host $Response
Write-Host "ASB - /Requirements/SubmissionStatuses/ProgramOfficeUserChanges on" $which":  Test Failed " -f Red
exit 1
