...
#   Requirements query.
#
$argSet = addArguments -newArgName "EndDate" -newArgValue "10/01/2025"
$argSet = addArguments -sofar $argSet -newArgName "StartDate" -newArgValue "10/30/2025"
...
Write-Host $tname": Missing ProjectNumber parameter" -f magenta
$mine = ($Response | ConvertFrom-Json)
if ($mine.ResponseStatus.ErrorCode -eq 4003) {
   if ($mine.ResponseStatus.Message.Contains("ProjectNumber field is required")) {
