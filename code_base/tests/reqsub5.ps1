...
#
#
#$fulluri = Add_route -uri $myuri -route "Requirements/SubmissionStatuses/MostRecent" -query $idval
$fulluri = Add_route -uri $myuri -routeId 82 -query $idval
...
if ($Response.StatusCode -eq 200) {
   ...
   if ($mine.Result.Count -gt 0) {
      if ($mine.Result.RequirementId -eq $idval) {
         Write-Host "ASB - /Requirements/SubmissionStatuses/MostRecent on" $which":  Test Passed " -f Green
         exit 0
      }
   }
}
...
#Write-Host $Response
Write-Host "ASB - /Requirements/SubmissionStatuses/MostRecent on" $which":  Test Failed " -f Red
exit 1
