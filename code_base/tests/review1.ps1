...
#   Requirements query.
#
#$fulluri = Add_route -uri $myuri -route "/Lists/ReviewAssessment/Planning"
$fulluri = Add_route -uri $myuri -routeId 73
...
if ($Response.StatusCode -eq 200) {
   ...
   if ($mine.result[1].ReviewAssessmentId -eq "8") {
      Write-Host "ASB - /Lists/ReviewAssessment/Planning on" $which":  Test Passed " -f Green
      exit 0
   }
}

Write-Host $Response
Write-Host "ASB - /Lists/ReviewAssessment/Planning on" $which":  Test Failed " -f Red
exit 1
