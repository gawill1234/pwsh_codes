...
$fulluri = Add_route -uri $myuri -routeId 53
...
$name_given = "OneMoreTime"
$body = '{"CategoryName": "' + $name_given + '"}'

$Response = run_uri_post -uri $fulluri -headers $headers -body $body
...
if ($Response.StatusCode -eq 200) {
   ...
   if ($mine.result.Name -eq $name_given) {
      Write-Host "ASB - /OAS/Categories on" $which":  Test Passed " -f Green
      exit 0
   } else {
...
Write-Host "ASB - /OAS/Categories on" $which":  Test Failed " -f Red
exit 1
