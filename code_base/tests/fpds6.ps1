...
#   Requirements query.
#
#$fulluri = Add_route -uri $myuri -route "/Lists/FpdsIdcType"
$fulluri = Add_route -uri $myuri -routeId 68
...
if ($Response.StatusCode -eq 200) {
   ...
   Write-Host "ASB - /List/FpdsIdcType on" $which":  Count =" $mine.result.Count -f Green
   Write-Host "ASB - /List/FpdsIdcType on" $which":  Name(checked) =" $mine.result[1].Name -f Green
   if ($mine.result.Count -eq 3) {
      if ($mine.result[1].Name -eq "IDC/Indefinite Quantity") {
         Write-Host "ASB - /List/FpdsIdcType on" $which":  Test Passed " -f Green
         exit 0
      }
   }
}
...
Write-Host $Response
Write-Host "ASB - /List/FpdsIdcType on" $which":  Test Failed " -f Red
exit 1
