...
#   Requirements query.
#
#$fulluri = Add_route -uri $myuri -route "/Lists/NaicsCode/Planning"
$fulluri = Add_route -uri $myuri -routeId 71
...
if ($Response.StatusCode -eq 200) {
   ...
   Write-Host "ASB - /List/NaicsCode/Planning on" $which":  Count =" $mine.result.Count -f Green
   if ($mine.result.Count -ge 1000) {
      $resstring = "Potato Farming"
      $locationval = ""
      $locationval = findResultItem -myjson $mine -lookingfor $resstring -what "Name"
      if ($locationval -ge 0) {
         Write-Host "ASB - /List/NaicsCode/Planning on" $which":  Name(checked) =" $mine.result[$locationval].Name -f Green
         if ($mine.result[$locationval].Name -eq $resstring) {
            Write-Host "ASB - /List/NaicsCode/Planning on" $which":  Test Passed " -f Green
            exit 0
         }
      }
   }
...
Write-Host $Response
Write-Host "ASB - /List/NaicsCode/Planning on" $which":  Test Failed " -f Red
exit 1
