...
#   Requirements query.
#
#$fulluri = Add_route -uri $myuri -route "/Lists/FpdsIdvType"
$fulluri = Add_route -uri $myuri -routeId 69
...
if ($Response.StatusCode -eq 200) {
   ...
   Write-Host "ASB - /List/FpdsIdvType on" $which":  Count =" $mine.result.Count -f Green
   Write-Host "ASB - /List/FpdsIdvType on" $which":  Name(checked) =" $mine.result[1].Name -f Green
   if ($mine.result.Count -eq 5) {
      if ($mine.result[1].Name -eq "IDC") {
         Write-Host "ASB - /List/FpdsIdvType on" $which":  Test Passed " -f Green
         exit 0
      }
   }
}
...
Write-Host $Response
Write-Host "ASB - /List/FpdsIdvType on" $which":  Test Failed " -f Red
exit 1
