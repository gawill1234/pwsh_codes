...
#   Requirements query.
#
#$fulluri = Add_route -uri $myuri -route "/Lists/ClassificationCode"
$fulluri = Add_route -uri $myuri -routeId 65
...
if ($Response.StatusCode -eq 200) {
   ...
   if ($mine.result[1].Name -eq "Nuclear Ordnance") {
      Write-Host "ASB - /List/ClassificationCode on" $which":  Test Passed " -f Green
      exit 0
   }
}

Write-Host $Response
Write-Host "ASB - /List/ClassificationCode on" $which":  Test Failed " -f Red
exit 1
