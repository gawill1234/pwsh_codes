...
#   Vendor query.
#
#$fulluri = Add_route -uri $myuri -route "/Force/ActiveContractingOfficeID"
$fulluri = Add_route -uri $myuri -routeId 2
...
if ($Response.StatusCode -eq 200) {
   ...
   if ($mine -ne $null) {
      if ($mine.ASB_User -eq "swagger-user") {
         Write-Host "ASB - Force/ActiveContractingOfficeID on" $which":  Test Passed " -f Green
         exit 0
      }
   } else {
...
Write-Host $Response
Write-Host "ASB - Force/ActiveContractingOfficeID on" $which":  Test Failed " -f Red
exit 1
