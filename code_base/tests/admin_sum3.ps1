...
#   Requirements query.
#
#$fulluri = Add_route -uri $myuri -route "/Admin/Jobs/History/Summary"
$fulluri = Add_route -uri $myuri -routeId 6
...
Write-Host $tname": No date provided (beginning of time)" -f magenta
   ...
   if ($mine.result[0] -ne $null) {
      ...
      if ($mine.result.Count -eq 1) {
         ...
         if ($mine.result.Overall.Succeeded -ge 0) {
            Write-Host $tname":  Empty return, but got a result" -f Green
            Write-Host $tname": ASB - /Admin/Jobs/History/Summary on" $which":  Test Passed " -f Green
            exit 0
         }
      }
