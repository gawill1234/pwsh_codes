...
$fulluri = Add_route -uri $myuri -routeId 105 -query 1
...
Write-Host "Basic get of specific entry" -f magenta
if ($Response.StatusCode -eq 200) {
   $mine = ($Response.Content | ConvertFrom-Json)
   if ($mine.result.Name -eq "Primary Work") {
      Write-Host "ASB - /List/EmailAddressType on" $which":  Test Passed " -f Green
      exit 0
   }
}

Write-Host $Response
Write-Host "ASB - /List/EmailAddressType on" $which":  Test Failed " -f Red
exit 1
