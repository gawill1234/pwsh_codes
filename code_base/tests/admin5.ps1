...
#   Requirements query.
#
$argSet = addArguments -newArgName "Filter" -newArgValue "not-bloody-real"
...
if ($mine.ResponseStatus.ErrorCode -eq 4004) {
   if ($mine.ResponseStatus.Message.Contains("not valid for Filter")) {
      Write-Host $tname": ASB - /Admin/Jobs/History (Bad Filter) on" $which":  Test Passed " -f Green
      exit 0
   }
}

Write-Host $Response
Write-Host $tname": ASB - /Admin/Jobs/History (Bad Filter) on" $which":  Test Failed " -f Red
exit 1
