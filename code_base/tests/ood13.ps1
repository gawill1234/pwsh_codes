...
#   Requirements query.
#
#$fulluri = Add_route -uri $myuri -route "/OAS/OOD/Groups"
$fulluri = Add_route -uri $myuri -routeId 15
   ...
   if ($mine.result[0] -ne $null) {
      try {
         if ([int]$mine.result[0].Id -is [int]) {
            Write-Host "ASB - /OAS/OOD/Groups on" $which":  Test Passed " -f Green
            exit 0
         }
      } catch {
...
# Write-Host $Response
Write-Host "ASB - /OAS/OOD/Groups on" $which":  Test Failed " -f Red
exit 1
