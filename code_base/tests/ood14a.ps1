...
#   Requirements query.
#
#$fulluri = Add_route -uri $myuri -route "/OAS/OOD/Groups" -query 9000
$fulluri = Add_route -uri $myuri -routeId 15 -query 9000
...
Write-Host $tname": Contract Id does not exist" -f magenta
if ($Response.StatusCode -ne 200) {
   $mine = ($Response | ConvertFrom-Json)
   if ($mine.ResponseStatus.Message.Contains("contract group was not found")) {
