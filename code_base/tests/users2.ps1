...
# TODO: replace witha test user email
$email = "thomas.gadacy@va.gov"

$argSet = addArguments -newArgName "EmailAddress" -newArgValue $email
#
#   Add on the actual route/api part.  The specific
#   bit that says what to do.  This is a very simple
#   Requirements query.
#
$fulluri = Add_route -uri $myuri -routeId 121 -query $argSet
...
if ($Response.StatusCode -eq 200) {
   ...
   if ($mine.result.User.Communication.EmailAddress -eq $email) {
      Write-Host "ASB - /Users/ByEmailAddress" $which":  Test Passed " -f Green
      exit 0
   }
}

Write-Host $Response
Write-Host "ASB - /Users/ByEmailAddress" $which":  Test Failed " -f Red
