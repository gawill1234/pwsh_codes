...
$tname = "ecor11_post.ps1"

$stuff = '{
   "Index": {
      "Id": 0,
      "ParentFolderEnumId": 41,
      "Name": "ecor11_post",
      "OrderBaseFolder": false,
      "DisplayOrder": 0,
      "ActivationDate": "2025-11-19",
      "DeactivationDate": "",
      "CurrentTags": []
   }
}'
...
#   Vendor query.
#
#$fulluri = Add_route -uri $myuri -route "OAS/Ecor/Indices"
$fulluri = Add_route -uri $myuri -routeId 29
...
if ($Response.StatusCode -eq 200) {
   ...
   if ($mine.result.Name -eq "ecor11_post") {
      Write-Host $tname": ASB - OAS/ECOR/Indices id: " $mine.result.Id -f Green
      #
      #   Create the data file needed by ecor12_del.ps1
      #
      $mine.result.Id | Out-File -FilePath .\ecor12.dat
      Write-Host $tname": ASB - OAS/ECOR/Indices on" $which":  Test Passed " -f Green
      exit 0
   }
}

Write-Host $Response
Write-Host $tname": ASB - OAS/ECOR/Indices on" $which":  Test Failed " -f Red
exit 1
