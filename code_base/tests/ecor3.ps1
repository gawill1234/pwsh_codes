...
#   Vendor query.
#
#$fulluri = Add_route -uri $myuri -route "OAS/Ecor/Reports" -query $qry
$fulluri = Add_route -uri $myuri -routeId 36 -query $qry
...
if ($Response.StatusCode -eq 200) {
   ...
   if ($mine.result[0].Id -eq $res) {
      Write-Host "ASB - OAS/ECOR/Reports on" $which":  Test Passed " -f Green
      exit 0
   }
}

Write-Host $Response
Write-Host "ASB - OAS/ECOR/Reports on" $which":  Test Failed " -f Red
exit 1
