...
#   Requirements query.
#
#$fulluri = Add_route -uri $myuri -route "/OAS/OOD/Groups" -query "abcde"
$fulluri = Add_route -uri $myuri -routeId 15 -query "abcde"
...
Write-Host $tname": Bad Contract Id format" -f magenta
if ($Response.StatusCode -ne 200) {
   $mine = ($Response | ConvertFrom-Json)
   if ($mine.ResponseStatus.Message.Contains("not valid for Id")) {
