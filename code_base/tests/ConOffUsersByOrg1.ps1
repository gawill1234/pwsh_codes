...
#   Requirements query.
#
#$fulluri = Add_route -uri $myuri -route "/Lists/ContractOfficeUsersByOrg" -query "3"
$fulluri = Add_route -uri $myuri -routeId 63 -query "3"
...
if ($Response.StatusCode -eq 200) {
   ...
   if ($mine.result[$loc].Name -eq $ans) {
      Write-Host "ASB - /List/ContractOfficeUsersByOrg on" $which":  Test Passed " -f Green
      exit 0
   } else {
      if ($mine.result.Count -gt 0) {
         Write-Host "ASB - /List/ContractOfficeUsersByOrg alternative check" -f Red
         Write-Host "ASB - /List/ContractOfficeUsersByOrg on" $which":  Test Passed " -f Green
         exit 0
      }
   }
...
Write-Host $Response
Write-Host "ASB - /List/ContractOfficeUsersByOrg on" $which":  Test Failed " -f Red
exit 1
