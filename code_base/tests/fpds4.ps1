...
#   Requirements query.
#
#$fulluri = Add_route -uri $myuri -route "/Lists/FpdsExtentCompeted"
$fulluri = Add_route -uri $myuri -routeId 67
...
if ($Response.StatusCode -eq 200) {
   ...
   Write-Host "ASB - /List/FpdsExtentCompeted on" $which":  Count =" $mine.result.Count -f Green
   Write-Host "ASB - /List/FpdsExtentCompeted on" $which":  Name(checked) =" $mine.result[1].Name -f Green
   if ($mine.result.Count -eq 9) {
      if ($mine.result[1].Name -eq "Not Available for Competition") {
         Write-Host "ASB - /List/FpdsExtentCompeted on" $which":  Test Passed " -f Green
         exit 0
      }
   }
}
...
Write-Host $Response
Write-Host "ASB - /List/FpdsExtentCompeted on" $which":  Test Failed " -f Red
exit 1
