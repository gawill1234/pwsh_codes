...
#   Requirements query.
#
$argSet = addArguments -newArgName "finkleStein" -newArgValue "10/01/2025"
...
Write-Host $tname": Bad parameter name provided" -f magenta
if ($Response.StatusCode -eq 200) {
   $mine = ($Response.Content | ConvertFrom-Json)
   if ($mine.result[0] -ne $null) {
      Write-Host $tname": ASB - /Admin/Jobs/History/Summary count:" $mine.result.Count -f Red
      if ($mine.result.Count -eq 1) {
         if ($mine.result.Overall.Succeeded -ge 1) {
            Write-Host $tname": ASB - /Admin/Jobs/History/Summary on" $which":  Test Failed " -f Red
            exit 1
         }
         if ($mine.result.Overall.Succeeded -ge 0) {
            Write-Host $tname": Got a result with a bad parameter" -f Red
            Write-Host $tname": ASB - /Admin/Jobs/History/Summary on" $which":  Test Failed " -f Red
            exit 1
         }
      }
...
Write-Host $Response
Write-Host $tname": ASB - /Admin/Jobs/History/Summary on" $which":  Test Passed " -f Green
exit 1
