...
#   Requirements query.
#
#$fulluri = Add_route -uri $myuri -route "/Organizations/ApplicationFeatures" -query 3
$fulluri = Add_route -uri $myuri -routeId 76 -query 3
...
if ($Response.StatusCode -eq 200) {
   ...
   if ($mine.result.Name -eq "VA National Cemetery Administration (NCA)") {
      Write-Host "ASB - /Organizations/ApplicationFeatures on" $which":  Test Passed " -f Green
      exit 0
   }
}

Write-Host $Response
Write-Host "ASB - /Organizations/ApplicationFeatures on" $which":  Test Failed " -f Red
exit 1
