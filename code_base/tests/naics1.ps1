...
#   Requirements query.
#
#$fulluri = Add_route -uri $myuri -route "/Lists/NaicsCode"
$fulluri = Add_route -uri $myuri -routeId 72
...
if ($Response.StatusCode -eq 200) {
   ...
   Write-Host "ASB - /List/NaicsCode on" $which":  Count =" $mine.result.Count -f Green
   Write-Host "ASB - /List/NaicsCode on" $which":  Name(checked) =" $mine.result[9].Name -f Green
   if ($mine.result.Count -ge 2000) {
      $resstring = "Real Estate And Rental And Leasing"
