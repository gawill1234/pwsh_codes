...
#   Requirements query.
#
$argSet = addArguments -newArgName "FromDate" -newArgValue "notadatestring"
...
Write-Host $tname": Bad date (no digits)" -f magenta
$mine = ($Response | ConvertFrom-Json)
if ($mine.ResponseStatus.ErrorCode -eq 4004) {
   if ($mine.ResponseStatus.Message.Contains("not valid for FromDate")) {
      Write-Host $tname": ASB - /Admin/Jobs/History/Summary (Bad Date) on" $which":  Test Passed " -f Green
      exit 0
   }
}

Write-Host $Response
Write-Host $tname": ASB - /Admin/Jobs/History/Summary (Bad Date) on" $which":  Test Failed " -f Red
exit 1
