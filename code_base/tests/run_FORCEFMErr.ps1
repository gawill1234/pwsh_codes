...
#   Vendor query.
#
$argSet = addArguments -newArgName "OrganizationId" -newArgValue "3"
$argSet = addArguments -sofar $argSet -newArgName "ViewContext" -newArgValue "VACM"
#$fulluri = Add_route -uri $myuri -route "/Force/FormModel" -query "OrganizationId=3&ViewContext=VACM"
$fulluri = Add_route -uri $myuri -routeId 0 -query "OrganizationId=3&ViewContext=VACM"
...
if ($mine.ResponseStatus.ErrorCode -eq "NullReferenceException") {
      Write-Host "ASB - /Force/FormModel on" $which -f Red
      Write-Host "  Got a server error.  Should be an empty list." -f Red
      Write-Host "  But we accept this response for historical reasons" -f Red
      Write-Host "ASB - Force/FormModel is expected to have an error" -f Green
      Write-Host "ASB - Force/FormModel on" $which":  Test Passed " -f Green
      exit 0
}


Write-Host $Response
Write-Host "ASB - Force/FormModel on" $which":  Test Failed " -f Red
exit 1
