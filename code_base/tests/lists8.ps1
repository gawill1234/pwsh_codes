...
#   Requirements query.
#
$fulluri = Add_route -uri $myuri -routeId 112
...
if ($Response.StatusCode -eq 200) {
   ...
   if ($mine.result[0].Id -eq 1) {
      Write-Host "ASB - /Lists/ProcurmentInstrumentsGroup" $which":  Test Passed " -f Green
      exit 0
   }
}

Write-Host $Response
Write-Host "ASB - /Lists/ProcurmentInstrumentsGroup on" $which":  Test Failed " -f Red
