...
#   Requirements query.
#
$fulluri = Add_route -uri $myuri -routeId 106
...
if ($Response.StatusCode -eq 200) {
   ...
   if ($mine.result[0].Id -eq "A") {
      Write-Host "ASB - /Lists/ProcurmentInstruments on" $which":  Test Passed " -f Green
      exit 0
   }
}

Write-Host $Response
Write-Host "ASB - /Lists/ProcurmentInstruments on" $which":  Test Failed " -f Red
