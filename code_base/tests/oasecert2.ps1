...
#   Vendor query.
#
#$fulluri = Add_route -uri $myuri -route "OAS/Ecert/Accreditations"
$fulluri = Add_route -uri $myuri -routeId 41
...
if ($Response.StatusCode -eq 200) {
   ...
   if ($mine.result[0] -ne $null) {
      Write-Host $tname": ASB - OAS/ECERT/Accreditations count: " $mine.result.Count -f Green
      Write-Host $tname": ASB - OAS/ECERT/Accreditations on" $which":  Test Passed " -f Green
      exit 0
   } else {
...
Write-Host $Response
Write-Host $tname": ASB - OAS/ECERT/Accreditations on" $which":  Test Failed " -f Red
exit 1
