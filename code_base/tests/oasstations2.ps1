...
#   Vendor query.
#
#$fulluri = Add_route -uri $myuri -route "OAS/Locations/Stations" -query 54
$fulluri = Add_route -uri $myuri -routeId 51 -query 54
...
if ($Response.StatusCode -eq 200) {
   ...
   if ($mine.result[0].LocationId -eq 54) {
      Write-Host $tname": ASB - OAS/Locations/Stations station code:" $mine.result[1].StationCode -f Green
