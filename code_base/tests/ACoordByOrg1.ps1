...
#   Requirements query.
#
#$fulluri = Add_route -uri $myuri -route "/Lists/AssignmentCoordinatorsByOrg" -query "3"
$fulluri = Add_route -uri $myuri -routeId 62 -query "3"
...
if ($Response.StatusCode -eq 200) {
   ...
   if ($mine.result[$loc].Name -eq $name) {
      Write-Host "ASB - /List/AssignmentCoordinatorsByOrg on" $which":  Test Passed " -f Green
      exit 0
   } else {
      if ($mine.result.Count -gt 0) {
         Write-Host "ASB - /List/AssignmentCoordinatorsByOrg alternative check" -f Red
         Write-Host "ASB - /List/AssignmentCoordinatorsByOrg on" $which":  Test Passed " -f Green
         exit 0
      }
   }
...
Write-Host $Response
Write-Host "ASB - /List/AssignmentCoordinatorsByOrg on" $which":  Test Failed " -f Red
exit 1
