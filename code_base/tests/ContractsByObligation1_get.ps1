...
$tname = "ContractsByObligation1_get.ps1"
...
#   Requirements query.
#
$argSet = addArguments -newArgName "UpdatedFromDate"-newArgValue "01/11/2025"
$argSet = addArguments -sofar $argSet -newArgName "UpdatedToDate"-newArgValue "01/31/2025"

#$fulluri = Add_route -uri $myuri -route "/Contracts.v16-01/ByObligation" -query $argSet
$fulluri = Add_route -uri $myuri -routeId 100 -query2 $argSet
...
Write-Host $tname": Alternate date format (mm/dd/yyyy)" -f magenta
if ($Response.StatusCode -eq 200) {
   $mine = ($Response.Content | ConvertFrom-Json)
   if ($mine.Awards.Count -gt 0) {
      if ($mine.Awards[0].Identification.BasePIID -ne "" -and $mine.Awards[0].Identification.BasePIID -ne $null) {
         Write-Host "ASB - /Contracts.v16-01/ByObligation on" $which":  Test Passed " -f Green
         exit 0
      }
   }
}
...
# Write-Host $Response
Write-Host "ASB - /Contracts.v16-01/ByObligation on" $which":  Test Failed " -f Red
exit 1
