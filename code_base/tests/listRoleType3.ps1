...
#   Requirements query.
#
$myrand = Get-Random -Minimum 1 -Maximum 26
$argSet = addArguments -newArgName "Id"-newArgValue $myrand
$fulluri = Add_route -uri $myuri -routeId 107 -query $myrand
...
if ($Response.StatusCode -eq 200) {
   ...
   if ($mine.Result.Count -gt 0) {
         Write-Host "ASB - /Lists/RoleType.v16-01 on" $which":  Test Passed " -f Green
         exit 0
   }
}

# Write-Host $Response
Write-Host "ASB - /Lists/RoleType.v16-01 on" $which":  Test Failed " -f Red
exit 1
