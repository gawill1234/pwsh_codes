...
#$fulluri = Add_route -uri $myuri -route "/Admin/Jobs/History/Summary" -query $argSet
$fulluri = Add_route -uri $myuri -routeId 6 -query $argSet

$argSet = addArguments -newArgName "FromDate" -newArgValue "2025-10-01"
#$fulluri2 = Add_route -uri $myuri -route "/Admin/Jobs/History/Summary" -query $argSet
$fulluri2 = Add_route -uri $myuri -routeId 6 -query $argSet
...
Write-Host $tname": Compare result of: " -f magenta
Write-Host "URI:  $fulluri" -f magenta
Write-Host "URI:  $fulluri2" -f magenta
Write-Host $tname": Direct compare of two date format results" -f magenta
if ($Response.StatusCode -eq 200) {
   ...
   if ($mine.result -ne $null) {
      if ($mine.result.Count -eq $mine2.result.Count) {
         if ($mine.result.Overall.Succeeded -eq $mine2.result.Overall.Succeeded) {
            Write-Host $tname": ASB - /Admin/Jobs/History/Summary (FromDate slash) on" $which":  Test Passed " -f Green
            exit 0
         } else {
            Write-Host $tname": ASB - /Admin/Jobs/History on" $which":  Group compare fail " -f Green
         }
      } else {
            Write-Host $tname": ASB - /Admin/Jobs/History/Summary on" $which":  Count compare fail " -f Green
            Write-Host $tname": ASB - /Admin/Jobs/History/Summary count:" $mine.result.Count -f Green
            Write-Host $tname": ASB - /Admin/Jobs/History/Summary count:" $mine2.result.Count -f Green
      }
   }
}

Write-Host $Response
Write-Host $tname": ASB - /Admin/Jobs/History/Summary (FromDate slash) on" $which":  Test Failed " -f Red
exit 1
