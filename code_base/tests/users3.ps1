...
# TODO: Replaces below args with Test users account
$argSet = addArguments -newArgName "OrgId" -newArgValue 17
$argSet = addArguments -sofar $argSet -newArgName "RoleId" -newArgValue 2
#
#   Add on the actual route/api part.  The specific
#   bit that says what to do.  This is a very simple
#   Requirements query.
#
$fulluri = Add_route -uri $myuri -routeId 122 -query $argSet
...
if ($Response.StatusCode -eq 200) {
   ...
   if ($mine.result[0].UserTypeId -eq 1) {
      Write-Host "ASB - /Users/ByOrgAndRole" $which":  Test Passed " -f Green
      exit 0
   }
}

Write-Host $Response
Write-Host "ASB - /Users/ByOrgAndRole" $which":  Test Failed " -f Red
