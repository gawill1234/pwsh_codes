...
#   Vendor query.
#
#$fulluri = Add_route -uri $myuri -route "OAS/Ecert/User/Accreditations" -query 100
$fulluri = Add_route -uri $myuri -routeId 40 -query 100
...
if ($Response.StatusCode -eq 200) {
   ...
   if ($mine.result[0].DisplayName -eq "Amanda Phillips") {
      Write-Host $tname": ASB - OAS/Ecert/User/{id}/Accreditations count: " $mine.result.Count -f Green
