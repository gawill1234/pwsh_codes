...
#   Vendor query.
#
#$fulluri = Add_route -uri $myuri -route "OAS/Ecor/Issues"
$fulluri = Add_route -uri $myuri -routeId 30
...
if ($Response.StatusCode -eq 200) {
   ...
   Write-Host $tname": ASB - OAS/ECOR/Issues count: " $mine.result.Count -f Green
   if ($mine.result.Count -ge $res) {
      if ($mine.result[0].Id -ne "" -and $mine.result[0].Id -ne $null) {
         Write-Host $tname": ASB - OAS/ECOR/Issues ID: " $mine.result[0].Id -f Green
      }
      Write-Host $tname": ASB - OAS/ECOR/Issues on" $which":  Test Passed " -f Green
      exit 0
   }
}

Write-Host $Response
Write-Host $tname": ASB - OAS/ECOR/Issues on" $which":  Test Failed " -f Red
exit 1
