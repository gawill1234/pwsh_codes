...
$fulluri = Add_route -uri $myuri -routeId 108 -query 1
...
if ($Response.StatusCode -eq 200) {
   ...
   if ($mine.result.Name -eq "Draft") {
      Write-Host "ASB - /List/SubmissionStatus on" $which":  Test Passed " -f Green
      exit 0
   }
}

Write-Host $Response
Write-Host "ASB - /List/SubmissionStatus on" $which":  Test Failed " -f Red
exit 1
