...
#   Vendor query.
#
#$fulluri = Add_route -uri $myuri -route "OAS/Locations/Stations" -query "hello"
$fulluri = Add_route -uri $myuri -routeId 51 -query "hello"
...
Write-Host $tname": ASB - OAS/Locations/Stations bad station id" -f magenta
$mine = ($Response | ConvertFrom-Json)
if ($mine.ResponseStatus.ErrorCode -eq 4004) {
   if ($mine.ResponseStatus.Errors.Message.Contains("not valid for LocationId")) {
      Write-Host $tname": ASB - OAS/Locations/Stations on" $which":  Test Passed " -f Green
      exit 0
   }
}

Write-Host $Response
Write-Host $tname": ASB - OAS/Locations/Stations on" $which":  Test Failed " -f Red
exit 1
