...
#   Vendor query.
#
$fulluri = Add_route -uri $myuri -routeId 93 -query $reqId -query2 $docName
   ...
   foreach ($header in $mine.GetEnumerator()) {
      ...
      if ($header.key -eq "Content-Disposition") {
         if ($($header.Value).Contains("$docName")) {
            ...
            Write-Host $tname": ASB - /Documents/{id}/Artifacts/{doc} on" $which":  Test Passed " -f Green
            exit 0
         }
      }
   }
...
Write-Host $Response
Write-Host $tname": ASB - /Documents/{id}/Artifacts/{doc} on" $which":  Test Failed " -f Red
exit 1
