...
#   Requirements query.
#
$argSet = addArguments -newArgName "UserId" -newArgValue "1"
$argSet = addArguments -sofar $argSet -newArgName "GroupId" -newArgValue "3"
...
Write-Host $tname": UserId does not exist" -f magenta
if ($Response.StatusCode -ne 200) {
   $mine = ($Response | ConvertFrom-Json)
   if ($mine.ResponseStatus.Message.Contains("User must complete their profile")) {
