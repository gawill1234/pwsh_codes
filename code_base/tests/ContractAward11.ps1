...
#   Requirements query.
#
$argSet = addArguments -newArgName "OrderPIID" -newArgValue "V797A63634-S"

#$fulluri = Add_route -uri $myuri -route "/Contracts/Conformed" -query2 "SP0200-02-D-8314" -query $argSet
$fulluri = Add_route -uri $myuri -routeId 61 -query "SP0200-02-D-8314" -query2 $argSet
...
if ($Response.StatusCode -eq 200) {
   ...
   Write-Host $tname": Checking this item (ReferenceIDV) --" -f magenta
   Write-Host "        "$mine.Contracts[0].Identification.ReferenceIDV -f magenta
   if ($mine.Contracts[0].Identification.ReferenceIDV.Contains("SP0200-02-D-8314 ")) {
      Write-Host $tname": ASB - /Contracts/{BasePIID}/Conformed on" $which":  Test Passed " -f Green
      exit 0
   }
}

Write-Host $Response
Write-Host $tname": ASB - /Contracts/{BasePIID}/Conformed on" $which":  Test Failed " -f Red
exit 1
