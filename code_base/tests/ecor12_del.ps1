...
#
if (Test-Path -Path ".\ecor12.dat") {
   $dumpId = Get-Content -Path ".\ecor12.dat"
} else {
   Write-Host $tname": Id input file not found (ecor12.dat)" -f Red
   Write-Host $tname": Run ecor11_post.ps1 to create the file" -f Red
...
#   Vendor query.
#
#$fulluri = Add_route -uri $myuri -route "OAS/Ecor/Indices" -query $dumpId
$fulluri = Add_route -uri $myuri -routeId 29 -query $dumpId
