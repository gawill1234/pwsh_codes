...
#$fulluri = Add_route -uri $myuri -route "OAS/Ecor/Files" -query $qry
$fulluri = Add_route -uri $myuri -routeId 35 -query $qry
...
if ($Response.StatusCode -eq 200) {
   ...
   Write-Host "ASB - OAS/ECOR/Files count: " $mine.result.Count -f Green
   if ($mine.result.Count -ge $res) {
      if ($mine.result.Count -gt 0) {
         Write-Host "ASB - OAS/ECOR/Files ID: " $mine.result[0].Id -f Green
      }
      Write-Host "ASB - OAS/ECOR/Files on" $which":  Test Passed " -f Green
      exit 0
   }
   exit 0
}
...
Write-Host $Response
Write-Host "ASB - OAS/ECOR/Files on" $which":  Test Failed " -f Red
exit 1
