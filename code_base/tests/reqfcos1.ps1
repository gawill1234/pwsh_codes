...
#   Vendor query.
#
$fulluri = Add_route -uri $myuri -routeId 92 -query $reqId
...
if ($Response.StatusCode -eq 200) {
   ...
   if ($mine.result.Id -eq $reqId) {
      Write-Host $tname": ASB - /Requirements/Fcos on" $which":  Test Passed " -f Green
      exit 0
   }
}

Write-Host $Response
Write-Host $tname": ASB - /Requirements/Fcos on" $which":  Test Failed " -f Red
exit 1
