...
#$fulluri = Add_route -uri $myuri -route "/Admin/Jobs/History"
$fulluri = Add_route -uri $myuri -routeId 7
...
if ($Response.StatusCode -eq 200) {
   ...
   if ($mine.result[0] -ne $null) {
      ...
      if ($mine.result[0].JobGroup -eq $ans) {
         Write-Host $tname": ASB - /Admin/Jobs/History (no params) on" $which":  Test Passed " -f Green
         exit 0
      }
   } else {
...
Write-Host $Response
Write-Host $tname": ASB - /Admin/Jobs/History (no params) on" $which":  Test Failed " -f Red
exit 1
