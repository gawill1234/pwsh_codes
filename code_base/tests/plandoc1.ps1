...
#   Vendor query.
#
$fulluri = Add_route -uri $myuri -routeId 91 -query 3
   ...
   foreach ($header in $mine.GetEnumerator()) {
      ...
      if ($header.key -eq "Content-Disposition") {
         if ($($header.Value).Contains("filename=")) {
            ...
            Write-Host $tname": ASB - /Planning/Requirements/Operations/Documents on" $which":  Test Passed " -f Green
            exit 0
         }
      }
   }
...
Write-Host $Response
Write-Host $tname": ASB - /Planning/Requirements/Operations/Documents on" $which":  Test Failed " -f Red
exit 1
