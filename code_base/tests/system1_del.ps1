...
$tname = "system1_del.ps1"


#
#   Get the Id created by system1_post.ps1
#
if (Test-Path -Path ".\system1f.dat") {
   $dumpId = Get-Content -Path ".\system1f.dat"
} else {
   Write-Host $tname": Id input file not found (system1f.dat)" -f Red
   Write-Host $tname": Run system1_post.ps1 to create the file" -f Red
   Write-Host $tname": ASB - /OAS/System/Users on" $which":  Test Failed " -f Red
   exit 1
}
...
$fulluri = Add_route -uri $myuri -routeId 42 -query $dumpId
#
...
if ($Response.StatusCode -eq 200) {
   ...
   if ($mine.RemovedCount -eq 0) {
      if (Test-Path -Path ".\system1f.dat") {
         Remove-Item ".\system1f.dat" -Force
      }
      Write-Host $tname": ASB - /OAS/System/Users on" $which":  Test Passed " -f Green
      exit 0
   }
}
