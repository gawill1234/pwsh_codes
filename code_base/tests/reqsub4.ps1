...
#
#
#$fulluri = Add_route -uri $myuri -route "Requirements/SubmissionStatuses" -query $idval
$fulluri = Add_route -uri $myuri -routeId 83 -query $idval
...
if ($Response.StatusCode -eq 200) {
   ...
   if ($mine.Result.Count -gt 0) {
      if ($mine.Result[0].RequirementId -eq $idval) {
         Write-Host "ASB - /Requirements/SubmissionStatuses on" $which":  Test Passed " -f Green
         exit 0
      }
   }
}
...
#Write-Host $Response
Write-Host "ASB - /Requirements/SubmissionStatuses on" $which":  Test Failed " -f Red
exit 1
