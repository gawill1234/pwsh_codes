...
Write-Host $tname": Compare results of:" -f magenta
Write-Host $tname": URI:  $fulluri" -f magenta
Write-Host $tname": URI:  $fulluri_true" -f magenta
Write-Host $tname": Compare ShowArchived parameter (true vs false)" -f magenta
if ($Response.StatusCode -eq 200) {
   $mine = ($Response.Content | ConvertFrom-Json)
   $mine_true = ($Response_true.Content | ConvertFrom-Json)
   if ($mine.result.Count -le $mine_true.result.Count) {
      Write-Host $tname": ASB - OAS/Organizations count (false): " $mine.result.Count -f Green
      Write-Host $tname": ASB - OAS/Organizations count (true): " $mine_true.result.Count -f Green
