...
#   Requirements query.
#
$argSet = addArguments -newArgName "ProjectNumber" -newArgValue "649-B67007B"
...
Write-Host "Project Number parameter" -f magenta
if ($Response.StatusCode -eq 200) {
   $mine = ($Response.Content | ConvertFrom-Json)
   if ($mine.Result.Contracts[0].Award -ne $null) {
      if ($mine.Result.Contracts[0].Award[0].Info.Title.Contains("SIEMENS, XRAY1, REMAN")) {
