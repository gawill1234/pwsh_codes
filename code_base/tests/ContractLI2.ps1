...
#   Requirements query.
#
$argSet = addArguments -newArgName "BasePIID" -newArgValue "VANAC-90NP1-03-025"
$argSet = addArguments -sofar $argset -newArgName "ModificationNumber" -newArgValue "0005"

#$fulluri = Add_route -uri $myuri -route "/Contracts/LineItems/IDVLineItems" -query $argSet
$fulluri = Add_route -uri $myuri -routeId 59 -query $argSet
...
Write-Host "Both allowed parameters needed (BasePIID, ModificationNumber)" -f magenta
if ($Response.StatusCode -eq 200) {
   $mine = ($Response.Content | ConvertFrom-Json)
   if ($mine.Result.Contract.Info.Title.Contains("BIRD & CRONIN")) {
      Write-Host $tname": ASB - /Contracts/LineItems/IDVLineItems on" $which":  Test Passed " -f Green
      exit 0
   }
}

Write-Host $Response
Write-Host $tname": ASB - /Contracts/LineItems/IDVLineItems on" $which":  Test Failed " -f Red
exit 1
