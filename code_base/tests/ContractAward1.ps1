...
#   Requirements query.
#
#$fulluri = Add_route -uri $myuri -route "/Contracts/ActiveAwards"
$fulluri = Add_route -uri $myuri -routeId 57
...
if ($Response.StatusCode -eq 200) {
   ...
   if ($mine.ContractsAwarded.Count -gt 0) {
      if ($mine.ContractsAwarded[0].BasePIID -ne "" -and $mine.ContractsAwarded[0].BasePIID -ne $null) {
         Write-Host "ASB - /Contracts/ActiveAwards on" $which":  Test Passed " -f Green
         exit 0
      }
   }
}
...
# Write-Host $Response
Write-Host "ASB - /Contracts/ActiveAwards on" $which":  Test Failed " -f Red
exit 1
