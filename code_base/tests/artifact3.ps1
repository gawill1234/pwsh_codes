...
$reqId = 210
$docName = "VA-245-08-RP-0055-000.DOC"
$vers = 1
...
#   Vendor query.
#
$fulluri = Add_route -uri $myuri -routeId 94 -query $reqId -query2 $docName -query3 $vers
...
if ($Response.StatusCode -eq 200) {
   $mine = $Response.Headers
   foreach ($header in $mine.GetEnumerator()) {
      Write-Host "$($header.key):  $($header.Value)" -f magenta
      if ($header.key -eq "Content-Disposition") {
         if ($($header.Value).Contains("$docName")) {
            Write-Host $tname": $($header.Key): $($header.Value)"
            Write-Host $tname": ASB - /Documents/{id1}/Artifacts/{doc}/Version/{vers} on" $which":  Test Passed " -f Green
            exit 0
         }
      }
   }
...
Write-Host $Response
Write-Host $tname": ASB - /Documents/{id1}/Artifacts/{doc}/Version/{vers} on" $which":  Test Failed " -f Red
exit 1
