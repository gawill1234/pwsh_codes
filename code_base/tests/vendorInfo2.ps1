...
#   Vendor query.
#
$argSet = addArguments -newArgName "VendorName" -newArgValue "General Electric"
$argSet = addArguments -sofar $argSet -newArgName "VendorUEI" -newArgValue "DVNBAHCP4U66"
#$fulluri = Add_route -uri $myuri -route "VendorInfo" -query $argSet
$fulluri = Add_route -uri $myuri -routeId 87 -query $argSet
...
Write-Host "VendorName and VendorUEI parameters" -f magenta
if ($Response.StatusCode -eq 200) {
   $mine = ($Response.Content | ConvertFrom-Json)
   if ($mine.Result.Vendors[0].ContractorName.Contains("GENERAL ELECTRIC")) {
      Write-Host "ASB - Vendors/VendorInfo on" $which":  Test Passed " -f Green
      exit 0
   }
}

Write-Host $Response
Write-Host "ASB - Vendors/VendorInfo on" $which":  Test Failed " -f Red
exit 1
