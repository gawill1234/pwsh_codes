...
#   Requirements query.
#
$argSet = addArguments -newArgName "IncludeDisabled" -newArgValue "false"
#$fulluri = Add_route -uri $myuri -route "/Lists/ClassificationCode" -query $argSet
$fulluri = Add_route -uri $myuri -routeId 65 -query $argSet
...
Write-Host $tname": IncludeDisables param is false" -f magenta
if ($Response.StatusCode -eq 200) {
   $mine = ($Response.Content | ConvertFrom-Json)
   if ($mine.result[1].Name -eq "Nuclear Ordnance") {
      Write-Host $tname": ASB - /List/ClassificationCode on" $which":  Test Passed " -f Green
      exit 0
   }
}

Write-Host $Response
Write-Host $tname": ASB - /List/ClassificationCode on" $which":  Test Failed " -f Red
exit 1
