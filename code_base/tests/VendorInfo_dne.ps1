...
#   Vendor query.
#
$argSet = addArguments -newArgName "VendorName" -newArgValue "squirrel company"
...
if ($Response.StatusCode -eq 200) {
   if ($Response.Content.result.Vendors.Count -eq 0) {
      Write-Host "ASB - Vendors/VendorInfo (Does Not Exist) on" $which":  Test Passed " -f Green
      exit 0
   }
}

Write-Host $Response
Write-Host "ASB - Vendors/VendorInfo (Does Not Exist) on" $which":  Test Failed " -f Red
exit 1
