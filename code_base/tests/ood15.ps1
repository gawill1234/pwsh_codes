...
#   Requirements query.
#
#$fulluri = Add_route -uri $myuri -route "/OAS/OOD/Contracts/Appointments" -query $ans
$fulluri = Add_route -uri $myuri -routeId 27 -query $ans
...
if ($Response.StatusCode -eq 200) {
   ...
   if ($mine.Result[0].Vendor.Contains("MEDICAL PLACE INC")) {
      Write-Host "ASB - /OAS/OOD/Contracts/{id}/Appointments on" $which":  Test Passed " -f Green
      exit 0
   }
}

# Write-Host $Response
Write-Host "ASB - /OAS/OOD/Contracts/{id}/Appointments on" $which":  Test Failed " -f Red
exit 1
