...
#   Requirements query.
#
#$fulluri = Add_route -uri $myuri -route "/OAS/System/Users"
$fulluri = Add_route -uri $myuri -routeId 42
...
if ($Response.StatusCode -eq 200) {
   ...
   if ($mine.result.Email -eq "test@email.com") {
      Write-Host $tname": ASB - /OAS/System/Users id: " $mine.result.Id -f Green
      #
      #   Create the data file needed by system1_del.ps1
      #
      $mine.result.Id | Out-File -FilePath .\system1f.dat
...
Write-Host $Response
Write-Host "ASB - /OAS/System/Users on" $which":  Test Failed " -f Red
exit 1
