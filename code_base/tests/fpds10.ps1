...
#   Requirements query.
#
#$fulluri = Add_route -uri $myuri -route "/Lists/FpdsReasonForModification"
$fulluri = Add_route -uri $myuri -routeId 70
...
if ($Response.StatusCode -eq 200) {
   ...
   Write-Host "ASB - /List/FpdsReasonForModification on" $which":  Count =" $mine.result.Count -f Green
   Write-Host "ASB - /List/FpdsReasonForModification on" $which":  Name(checked) =" $mine.result[1].Name -f Green
   if ($mine.result.Count -eq 20) {
      if ($mine.result[1].Name -eq "Supp. Agreement for work") {
         Write-Host "ASB - /List/FpdsReasonForModification on" $which":  Test Passed " -f Green
         exit 0
      }
   }
}
...
Write-Host $Response
Write-Host "ASB - /List/FpdsReasonForModification on" $which":  Test Failed " -f Red
exit 1
