...
#   Requirements query.
#
#$fulluri = Add_route -uri $myuri -route "/Lists/NaicsCode" -query 372468
$fulluri = Add_route -uri $myuri -routeId 72 -query 372468
...
if ($Response.StatusCode -eq 200) {
   ...
   if ($mine.result.Count -eq 0) {
      Write-Host "ASB - /List/NaicsCode on" $which":  Test Passed " -f Green
      exit 0
   }
}
...
if ($Response.StatusCode -eq 200) {
   ...
   if ($mine.ResponseStatus.ErrorCode -eq "NullReferenceException") {
      Write-Host "ASB - /List/NaicsCode(does not exist) on" $which -f Red
      ...
      Write-Host "ASB - List/NaicsCode is expected to have an error" -f Red
      Write-Host "ASB - List/NaicsCode but we are still failing it" -f Red
   }
}

Write-Host $Response
Write-Host "ASB - /List/NaicsCode on" $which":  Test Failed " -f Red
exit 1
