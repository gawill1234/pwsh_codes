...
if ($dump -eq "yes") {
   $swine = ($Response.Content | ConvertFrom-Json | ConvertTo-Json)
   Write-Host $swine
}
...
if ($Response.StatusCode -eq 200) {
   ...
   if ($mine.result[0].Id -eq 1) {
      Write-Host "ASB - OAS/ECOR/Indices on" $which":  Test Passed " -f Green
      exit 0
   }
}

Write-Host $Response
Write-Host "ASB - OAS/ECOR/Indices on" $which":  Test Failed " -f Red
exit 1
