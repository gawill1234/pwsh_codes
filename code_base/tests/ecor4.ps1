...
#   Vendor query.
#
#$fulluri = Add_route -uri $myuri -route "OAS/Ecor/Files/Users" -query 11
$fulluri = Add_route -uri $myuri -routeId 34 -query 11
...
if ($Response.StatusCode -eq 200) {
   ...
   if ($mine.result[0].User.Email -eq "$res") {
      Write-Host "ASB - OAS/ECOR/Files/Users on" $which":  Test Passed " -f Green
      exit 0
   }
}

Write-Host $Response
Write-Host "ASB - OAS/ECOR/Files/Users on" $which":  Test Failed " -f Red
exit 1
