...
#   Requirements query.
#
$argSet = addArguments -newArgName "EndDate" -newArgValue "2025-10-01"
...
Write-Host $tname": Only one param (EndDate)" -f magenta
$mine = ($Response | ConvertFrom-Json)
if ($mine.ResponseStatus.ErrorCode -eq 4004) {
   if ($mine.ResponseStatus.Message.Contains("not be greater than 30")) {
