...
#   Requirements query.
#
#$fulluri = Add_route -uri $myuri -route "/OAS/OOD/NominationsCount" -query $qry
$fulluri = Add_route -uri $myuri -routeId 12 -query $qry
...
if ($Response.StatusCode -eq 200) {
   ...
   if ($mine.result[0] -ne $null) {
      if ($mine.result[0].Count -eq $res) {
         Write-Host "ASB - /OAS/OOD/NominationCount on" $which":  Test Passed " -f Green
         exit 0
      }
   } else {
...
Write-Host "ASB - /OAS/OOD/NominationCount on" $which":  Test Failed " -f Red
exit 1
