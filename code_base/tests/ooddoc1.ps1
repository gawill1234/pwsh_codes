...
#   Vendor query.
#
#$fulluri = Add_route -uri $myuri -route "OAS/OOD/DocumentTasks"
$fulluri = Add_route -uri $myuri -routeId 16
...
if ($Response.StatusCode -eq 200) {
   ...
   if ($mine.result[0] -eq $null) {
      Write-Host $tname": ASB - OAS/OOD/DocumentTasks count: " $mine.result.Count -f Green
      Write-Host $tname": ASB - OAS/OOD/DocumentTasks on" $which":  Test Passed " -f Green
      exit 0
   }
}

Write-Host $Response
Write-Host $tname": ASB - OAS/OOD/DocumentTasks on" $which":  Test Failed " -f Red
exit 1
