...
#   Requirements query.
#
$argSet = addArguments -newArgName "StartDate" -newArgValue "2025-13-01"
$argSet = addArguments -sofar $argSet -newArgName "EndDate" -newArgValue "2025-10-99"

#$fulluri = Add_route -uri $myuri -route "/Contracts/ActiveAwards/AwardedBetween" -query $argSet
$fulluri = Add_route -uri $myuri -routeId 56 -query $argSet
...
Write-Host $tname": Bad date" -f magenta
$mine = ($Response | ConvertFrom-Json)
if ($mine.ResponseStatus.ErrorCode -eq 4004) {
   if ($mine.ResponseStatus.Errors[1].Message.Contains("not valid for StartDate")) {
      Write-Host $tname": ASB - /Contracts/ActiveAwards/AwardedBetween (Bad Date) on" $which":  Test Passed " -f Green
      exit 0
   }
}

Write-Host $Response
Write-Host $tname": ASB - /Contracts/ActiveAwards/AwardedBetween (Bad Date) on" $which":  Test Failed " -f Red
exit 1
