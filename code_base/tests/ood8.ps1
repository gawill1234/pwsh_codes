...
#   Requirements query.
#
#$fulluri = Add_route -uri $myuri -route "/OAS/OOD/Appointments/History" -query $ans
$fulluri = Add_route -uri $myuri -routeId 21 -query $ans
...
if ($Response.StatusCode -eq 200) {
   ...
   if ($mine.result[0] -ne $null) {
      if ($mine.result[0].ContractUserArtifactId -eq $ans) {
         Write-Host "ASB - /OAS/OOD/Appointments/History on" $which":  Test Passed " -f Green
         exit 0
      }
   } else {
      Write-Host "The result was empty but the query succeeded" -f Yellow
      Write-Host "ASB - /OAS/OOD/Appointments on" $which":  Test Passed " -f Green
      exit 0
   }
}

Write-Host $Response
Write-Host "ASB - /OAS/OOD/Appointments/History on" $which":  Test Failed " -f Red
exit 1
