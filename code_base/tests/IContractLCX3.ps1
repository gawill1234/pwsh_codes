...
#   Requirements query.
#
$argSet = addArguments -newArgName "BasePIID" -newArgValue "SP0200-02-D-8314"
$argSet = addArguments -sofar $argset -newArgName "OrderPIID" -newArgValue "V797A73305"

#$fulluri = Add_route -uri $myuri -route "/Interface/Contracts/LifecycleExtended" -query $argSet
$fulluri = Add_route -uri $myuri -routeId 60 -query $argSet
...
if ($dump -eq "yes") {
   #Write-Host $Response
   Write-Host ($Response.Content | ConvertFrom-Json | ConvertTo-Json -Depth 11)
}
...
Write-Host "BasePIID and OrderPIID parameter combination" -f magenta
if ($Response.StatusCode -eq 200) {
   $mine = ($Response.Content | ConvertFrom-Json)
   if ($mine.Result.Contracts[0].Award -ne $null) {
      if ($mine.Result.Contracts[0].Award[0].Info.Title.Contains("SIEMENS, XRAY1")) {
         Write-Host $tname": ASB - Interface/Contracts/LifecycleExtended on" $which":  Test Passed " -f Green
         exit 0
      }
   }
}
...
# Write-Host $Response
Write-Host $tname": ASB - Interface/Contracts/LifecycleExtended on" $which":  Test Failed " -f Red
exit 1
