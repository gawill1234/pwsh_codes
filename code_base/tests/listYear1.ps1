...
$fulluri = Add_route -uri $myuri -routeId 110 -query2 $argSet
...
if ($Response.StatusCode -eq 200) {
   ...
   if ($mine.Result.Count -gt 0) {
         Write-Host "ASB - /Lists/Year.v16-01 on" $which":  Test Passed " -f Green
         exit 0
   }
}

# Write-Host $Response
Write-Host "ASB - /Lists/Year.v16-01 on" $which":  Test Failed " -f Red
exit 1
