...
#$fulluri = Add_route -uri $myuri -route "Requirements/SubmissionStatuses/AcquisitionPOCChanges" -query $argSet
$fulluri = Add_route -uri $myuri -routeId 80 -query $argSet
...
if ($Response.StatusCode -eq 200) {
   ...
   if ($mine.Result.Count -gt 0) {
      if ($mine.Result[0].ChangeOwner.Contains("$sv")) {
         Write-Host "ASB - /Requirements/SubmissionStatuses/AcquisitionPOCChanges on" $which":  Test Passed " -f Green
         exit 0
      }
   }
}
...
#Write-Host $Response
Write-Host "ASB - /Requirements/SubmissionStatuses/AcquisitionPOCChanges on" $which":  Test Failed " -f Red
exit 1
