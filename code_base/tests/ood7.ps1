...
#   Requirements query.
#
#$fulluri = Add_route -uri $myuri -route "/OAS/OOD/Appointments" -query $ans
$fulluri = Add_route -uri $myuri -routeId 20 -query $ans
...
if ($dump -eq "yes") {
   $klink = ($Response.Content | ConvertFrom-Json | ConvertTo-Json -Depth 5)
   Write-Host $klink
}
...
Write-Host $Response
Write-Host "ASB - /OAS/OOD/Appointments on" $which":  Test Failed " -f Red
exit 1
