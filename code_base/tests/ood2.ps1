...
#   Requirements query.
#
#$fulluri = Add_route -uri $myuri -route "/OAS/OOD/Contract/Nominations" -query $qry
$fulluri = Add_route -uri $myuri -routeId 25 -query $qry
...
if ($Response.StatusCode -eq 200) {
   ...
   if ($mine.result[0] -ne $null) {
      if ($mine.result[0].ContractId -eq $qry) {
         Write-Host "ASB - /OAS/OOD/Contract/{id}/Nominations on" $which":  Test Passed " -f Green
         exit 0
      }
   } else {
...
Write-Host "ASB - /OAS/OOD/Contract/{id}/Nominations on" $which":  Test Failed " -f Red
exit 1
