...
$fulluri = Add_route -uri $myuri -routeId 109 -params $argSet
...
Write-Host "Basic get with parameter" -f magenta
if ($Response.StatusCode -eq 200) {
   $mine = ($Response.Content | ConvertFrom-Json)
   if ($mine.result[0].Name -eq "Individual User") {
      Write-Host "ASB - /List/UserType on" $which":  Test Passed " -f Green
      exit 0
   }
}

Write-Host $Response
Write-Host "ASB - /List/UserType on" $which":  Test Failed " -f Red
exit 1
