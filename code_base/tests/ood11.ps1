...
#   Requirements query.
#
#$fulluri = Add_route -uri $myuri -route "/OAS/OOD/Search/Appointments" -query $ans
$fulluri = Add_route -uri $myuri -routeId 18 -query $ans
...
if ($dump -eq "yes") {
   $klink = ($Response | ConvertFrom-Json | ConvertTo-Json)
   Write-Host $klink
}

Write-Host "URI:  $fulluri" -f magenta
Write-Host "ASB - /OAS/OOD/Search/Appointments User does not exist" -f magenta
$mine = ($Response | ConvertFrom-Json)
if ($Response.StatusCode -ne 200) {
   if ($mine.ResponseStatus.Message.Contains("user does not exist")) {
      Write-Host "ASB - /OAS/OOD/Search/Appointments on" $which":  Test Passed " -f Green
      exit 0
   }
}

# Write-Host $Response
Write-Host "ASB - /OAS/OOD/Search/Appointments on" $which":  Test Failed " -f Red
exit 1
