...
#   Vendor query.
#
#$fulluri = Add_route -uri $myuri -route "OAS/Ecert/User/Accreditations" -query 9abc1
$fulluri = Add_route -uri $myuri -routeId 40 -query 9abc1
...
Write-Host "Invalid userid (string, not integer)" -f magenta
$mine = ($Response | ConvertFrom-Json)
if ($mine.ResponseStatus.ErrorCode -eq 4004) {
   if ($mine.ResponseStatus.Errors.Message.Contains("not valid for Id")) {
      Write-Host $tname": ASB - OAS/ECERT/User/{id}/Accreditations on" $which":  Test Passed " -f Green
      exit 0
   }
}

Write-Host $Response
Write-Host $tname": ASB - OAS/Ecert/User/{id}/Accreditations on" $which":  Test Failed " -f Red
exit 1
