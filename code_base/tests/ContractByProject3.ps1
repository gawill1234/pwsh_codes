...
$argSet = addArguments -sofar $argSet -newArgName "ProjectNumber" -newArgValue "5"
...
Write-Host $tname": Missing EndDate" -f magenta
$mine = ($Response | ConvertFrom-Json)
if ($mine.ResponseStatus.ErrorCode -eq 4003) {
   if ($mine.ResponseStatus.Message.Contains("EndDate field is required")) {
