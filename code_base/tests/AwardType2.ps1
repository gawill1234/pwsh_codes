...
#   Requirements query.
#
#$fulluri = Add_route -uri $myuri -route "/Lists/AwardType"
$fulluri = Add_route -uri $myuri -routeId 64
...
if ($Response.StatusCode -eq 200) {
   ...
   Write-Host "ASB - /List/AwardType on" $which":  Count =" $mine.result.Count -f Green
   Write-Host "ASB - /List/AwardType on" $which":  Name(checked) =" $mine.result[1].Name -f Green
   if ($mine.result.Count -eq 4) {
      if ($mine.result[1].Name -eq "Purchase Order") {
         Write-Host "ASB - /List/AwardType on" $which":  Test Passed " -f Green
         exit 0
      }
   }
}
...
Write-Host $Response
Write-Host "ASB - /List/AwardType on" $which":  Test Failed " -f Red
exit 1
