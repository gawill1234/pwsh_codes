...
#   Vendor query.
#
#$fulluri = Add_route -uri $myuri -route "OAS/OOD/User/DocumentTasks" -query 10417
$fulluri = Add_route -uri $myuri -routeId 17 -query 10417
...
Write-Host "Returns an empty set for now" -f magenta
if ($Response.StatusCode -eq 200) {
   $mine = ($Response.Content | ConvertFrom-Json)
   if ($mine.result[0] -eq $null) {
      Write-Host $tname": ASB - OAS/OOD/User/{id}/DocumentTasks count: " $mine.result.Count -f Green
      Write-Host $tname": ASB - OAS/OOD/User/{id}/DocumentTasks on" $which":  Test Passed " -f Green
      exit 0
   }
}

Write-Host $Response
Write-Host $tname": ASB - OAS/OOD/User/{id}/DocumentTasks on" $which":  Test Failed " -f Red
exit 1
