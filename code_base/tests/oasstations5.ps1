...
#   Vendor query.
#
#$fulluri = Add_route -uri $myuri -route "OAS/StationLocationPairs"
$fulluri = Add_route -uri $myuri -routeId 54
...
if ($Response.StatusCode -eq 200) {
   ...
   if ($mine.result.Count -ge 1) {
      if ($mine.result[0].LocationId -ne "" -and $mine.result[0].LocationId -ne $null) {
         Write-Host $tname": ASB - OAS/StationLocationPairs count:" $mine.result.Count -f Green
         Write-Host $tname": ASB - OAS/StationLocationPairs on" $which":  Test Passed " -f Green
         exit 0
      }
   }
}
...
Write-Host $Response
Write-Host $tname": ASB - OAS/StationLocationPairs on" $which":  Test Failed " -f Red
exit 1
