...
#   Requirements query.
#
#$fulluri = Add_route -uri $myuri -route "/OAS/OOD/Configuration/RequiredDocuments"
$fulluri = Add_route -uri $myuri -routeId 19
...
if ($Response.StatusCode -eq 200) {
   ...
   if ($mine.result[0] -ne $null) {
      try {
         if ([int]$mine.result[0].DocumentTypeEnumId -is [int]) {
            Write-Host "ASB - /OAS/OOD/Configuration/RequiredDocuments on" $which":  Test Passed " -f Green
            exit 0
         }
      } catch {
         Write-Host "The result ID was not an integer" -f Red
      }
   } else {
...
# Write-Host $Response
Write-Host "ASB - /OAS/OOD/Configuration/RequiredDocuments on" $which":  Test Failed " -f Red
exit 1
