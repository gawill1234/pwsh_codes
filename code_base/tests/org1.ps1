...
#   Requirements query.
#
#$fulluri = Add_route -uri $myuri -route "/Organizations"
$fulluri = Add_route -uri $myuri -routeId 75
...
if ($dump -eq "yes") {
   Write-Host ($Response | ConvertFrom-Json | ConvertTo-Json -Depth 7)
}

Write-Host "URI:  $fulluri" -f magenta
if ($Response.StatusCode -eq 200) {
   $mine = ($Response.Content | ConvertFrom-Json)
   Write-Host "ASB - /Organizations on" $which":  Count =" $mine.result.Count -f Green
   if ($mine.result.Count -ge 700) {
      $resstring = "Veterans Benefits Administration (VBA)"
      $locationval = findResultItem -myjson $mine -lookingfor $resstring -what "Name"
      if ($locationval -ge 0) {
         Write-Host "ASB - /Organizations on" $which":  Name(checked) =" $mine.result[$locationval].Name -f Green
         if ($mine.result[$locationval].Name -eq $resstring) {
            Write-Host "ASB - /Organizations on" $which":  Test Passed " -f Green
            exit 0
         }
      }
   }
...
Write-Host $Response
Write-Host "ASB - /Organizations on" $which":  Test Failed " -f Red
exit 1
