...
$routeerrors = 0
#
# Iterate over the list of queries and send a request
#
foreach ($query in $routequeries) {
   #
   #   Add on the actual route/api part.  The specific
   #   bit that says what to do.  This is a very simple
   #   Requirements query.
   #
   $fulluri = Add_route -uri $myuri -routeId 106 -query $query
   ...
   if ($Response.StatusCode -eq 200) {
      ...
      if ($mine.result[0].Id -eq $query) {
         Write-Host "ASB - /Lists/ProcurementInstrument on" $which":  Test Passed " -f Green
      }   
   } else {
      Write-Host $Response
      Write-Host "ASB - /Lists/ProcurementInstrument on" $which":  Test Failed " -f Red
      $routeerrors += 1
   }
}
...
#
# Error occured in one or more queries
#
exit 1
