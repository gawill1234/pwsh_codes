...
#   Requirements query.
#
$argSet = addArguments -newArgName "UserId" -newArgValue "10"
$argSet = addArguments -sofar $argSet -newArgName "GroupId" -newArgValue "99"
#$fulluri = Add_route -uri $myuri -route "/OAS/OOD/InitializeMassNomination" -query $argSet
$fulluri = Add_route -uri $myuri -routeId 13 -query $argSet
...
Write-Host $tname": GroupId does not exist" -f magenta
if ($Response.StatusCode -ne 200) {
   $mine = ($Response | ConvertFrom-Json)
   if ($mine.ResponseStatus.Message.Contains("requested group does not exist")) {
      Write-Host $tname": ASB - /OAS/OOD/InitializeMassNomination on" $which":  Test Passed " -f Green
      exit 0
   }
}

Write-Host $Response
Write-Host $tname": ASB - /OAS/OOD/InitializeMassNomination on" $which":  Test Failed " -f Red
exit 1
