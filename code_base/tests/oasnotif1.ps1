...
#   Vendor query.
#
#$fulluri = Add_route -uri $myuri -route "OAS/NotificationTemplates"
$fulluri = Add_route -uri $myuri -routeId 45
...
if ($Response.StatusCode -eq 200) {
   ...
   if ($mine.result.Count -ge 1) {
      if ($mine.result[1].FromEmail.Contains("@va.gov")) {
         Write-Host $tname": ASB - OAS/Notifications count: " $mine.result.Count -f Green
         Write-Host $tname": ASB - OAS/Notifications on" $which":  Test Passed " -f Green
         exit 0
      }
   }
}
...
Write-Host $Response
Write-Host $tname": ASB - OAS/Notifications on" $which":  Test Failed " -f Red
exit 1
