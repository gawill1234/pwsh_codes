...
#   Vendor query.
#
$fulluri = Add_route -uri $myuri -routeId 98
...
if ($Response.StatusCode -eq 200) {
   ...
   if ($mine.result[0].Message.Contains("Do not send")) {
      Write-Host "ASB - /OAS/ECOR/Preferences/History on" $which":  Test Passed " -f Green
      exit 0
   }
}

Write-Host $Response
Write-Host "ASB - /OAS/ECOR/Preferences/History on" $which":  Test Failed " -f Red
exit 1
