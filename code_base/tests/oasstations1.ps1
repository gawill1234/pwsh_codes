...
#   Vendor query.
#
#$fulluri = Add_route -uri $myuri -route "OAS/Stations"
...
if ($Response.StatusCode -eq 200) {
   ...
   if ($mine.result[1].StationCode -ne "" -and $mine.result[1].StationCode -ne $null) {
      Write-Host $tname": ASB - OAS/Stations station code:" $mine.result[1].StationCode -f Green
