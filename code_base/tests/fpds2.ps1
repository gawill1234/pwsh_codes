...
#   Requirements query.
#
#$fulluri = Add_route -uri $myuri -route "/Lists/FpdsContractingOfficeCode"
$fulluri = Add_route -uri $myuri -routeId 66
...
if ($Response.StatusCode -eq 200) {
   ...
   Write-Host "ASB - /List/FpdsContractingOfficeCode on" $which":  Count =" $mine.result.Count -f Green
   Write-Host "ASB - /List/FpdsContractingOfficeCode on" $which":  Name(checked) =" $mine.result[1].Name -f Green
   if ($mine.result.Count -gt 300) {
      if ($mine.result[1].Name -eq "TECHNOLOGY ACQUISITION CENTER") {
         Write-Host "ASB - /List/FpdsContractingOfficeCode on" $which":  Test Passed " -f Green
         exit 0
      }
   }
}
...
Write-Host $Response
Write-Host "ASB - /List/FpdsContractingOfficeCode on" $which":  Test Failed " -f Red
exit 1
