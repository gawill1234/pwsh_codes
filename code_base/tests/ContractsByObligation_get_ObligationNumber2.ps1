...
$tname = "ContractsByObligation_get_ObligationNumber2.ps1"
...
if ($Response.StatusCode -eq 200) {
   ...
   if ($mine.Awards.Count -gt 0) {
      if ($mine.Awards[0].Identification.BasePIID -ne "" -and $mine.Awards[0].Identification.BasePIID -ne $null) {
         Write-Host "ASB - /Contracts/ByObligation.v16-01 on" $which":  Test Passed " -f Green
         exit 0
      }
   }
}
...
# Write-Host $Response
Write-Host "ASB - /Contracts/ByObligation.v16-01 on" $which":  Test Failed " -f Red
exit 1
