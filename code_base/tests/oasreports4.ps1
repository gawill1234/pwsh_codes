...
#   Vendor query.
#
#$fulluri = Add_route -uri $myuri -route "OAS/Reports" -query 909090
$fulluri = Add_route -uri $myuri -routeId 47 -query 909090
...
Write-Host $tname": ASB - OAS/Reports report id does not exist" -f magenta
if ($Response.StatusCode -eq 200) {
   $mine = ($Response.Content | ConvertFrom-Json)
   if ($mine.result.Count -eq 0) {
      Write-Host $tname": ASB - OAS/Reports on" $which":  Test Passed " -f Green
      exit 0
   }
} else {
   $mine = ($Response | ConvertFrom-Json)
   if ($mine.ResponseStatus.ErrorCode -eq "ArgumentException") {
      if ($mine.ResponseStatus.Message -eq "The report could not be found.") {
         Write-Host $tname": ASB - OAS/Reports Should be empty result" -f Green
         Write-Host $tname": ASB - OAS/Reports but error is historically" -f Green
         Write-Host $tname": ASB - OAS/Reports what it has been" -f Green
         Write-Host $tname": ASB - OAS/Reports on" $which":  Test Passed " -f Green
         exit 0
      }
   }
...
Write-Host $Response
Write-Host $tname": ASB - OAS/Reports on" $which":  Test Failed " -f Red
exit 1
