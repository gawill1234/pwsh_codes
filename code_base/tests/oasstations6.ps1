...
#   Vendor query.
#
#$fulluri = Add_route -uri $myuri -route "OAS/Stations/Locations" -query 492
$fulluri = Add_route -uri $myuri -routeId 43 -query 492
...
if ($Response.StatusCode -eq 200) {
   ...
   if ($mine.result[0].LocationId -eq 1226) {
      Write-Host $tname": ASB - OAS/Stations/Locations station code:" $mine.result[0].Name -f Green
