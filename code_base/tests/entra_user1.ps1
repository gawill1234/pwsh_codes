...
$tname = "entra_user1.ps1"

. ../lib/appstatus.ps1

if ($isEntra -ne "yes") {
   Write-Host $tname": This test is Entra only" -f Yellow
   Write-Host $tname": Disabled for this and not run" -f Yellow
   Write-Host $tname": Test Passed" -f Green
   exit 0
}
...
$myguid = "8107a376-79dc-41af-b640-3219b9b5f564"

$argSet = addArguments -newArgName "EntraObjectId" -newArgValue $myguid
#
#   Add on the actual route/api part.  The specific
#   bit that says what to do.  This is a very simple
#   Vendor query.
#
#$fulluri = Add_route -uri $myuri -route "OAS/Users/Entra" -query $myguid
$fulluri = Add_route -uri $myuri -routeId 38 -query $argSet
...
$headers = build_headers -headers $headers -key "X-OAS-UserGUID" -value $myguid
...
if ($Response.StatusCode -eq 200) {
   ...
   if ($mine.result.Contact.Email -eq "Gary.Williams1@va.gov") {
      Write-Host $tname": ASB - OAS/Users/Entra on" $which":  Test Passed " -f Green
      exit 0
   }
}

Write-Host $Response
Write-Host $tname": ASB - OAS/Users/Entra on" $which":  Test Failed " -f Red
exit 1
