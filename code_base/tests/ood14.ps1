...
#   Requirements query.
#
#$fulluri = Add_route -uri $myuri -route "/OAS/OOD/Groups" -query 2
$fulluri = Add_route -uri $myuri -routeId 15 -query 2
...
if ($Response.StatusCode -eq 200) {
   ...
   if ($mine.result[0] -ne $null) {
      try {
         if ($mine.result.Title -eq "Pharmaceutical") {
            Write-Host "ASB - /OAS/OOD/Groups/{id} on" $which":  Test Passed " -f Green
            exit 0
         }
      } catch {
...
# Write-Host $Response
Write-Host "ASB - /OAS/OOD/Groups/{id} on" $which":  Test Failed " -f Red
exit 1
