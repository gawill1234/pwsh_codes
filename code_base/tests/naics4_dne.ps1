...
#   Requirements query.
#
#$fulluri = Add_route -uri $myuri -route "/Lists/NaicsCode/Planning" -query 90909 
$fulluri = Add_route -uri $myuri -routeId 71 -query 90909 
...
if ($Response.StatusCode -eq 200) {
   ...
   if ($mine.result.Count -eq 0) {
      Write-Host "ASB - /List/NaicsCode/Planning (does not exist) on" $which":  Test Passed " -f Green
      exit 0
   }
} else {
   $mine = ($Response | ConvertFrom-Json)
   if ($mine.ResponseStatus.ErrorCode -eq "NullReferenceException") {
      Write-Host "ASB - /List/NaicsCode/Planning (does not exist) on" $which -f Red
      ...
      Write-Host "ASB - List/NaicsCode/Planning is expected to have an error" -f Red
      Write-Host "ASB - List/NaicsCode/Planning but we are still failing it" -f Red
   }
}

Write-Host $Response
Write-Host "ASB - /List/NaicsCode/Planning (does not exist) on" $which":  Test Failed " -f Red
exit 1
