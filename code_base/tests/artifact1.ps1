...
#   Vendor query.
#
$fulluri = Add_route -uri $myuri -routeId 93 -query $reqId
...
if ($Response.StatusCode -eq 200) {
   ...
   if ($mine.result[0].DocumentID -eq $reqId) {
      Write-Host $tname": ASB - /Documents/{id}/Artifacts on" $which":  Test Passed " -f Green
      exit 0
   }
}

Write-Host $Response
Write-Host $tname": ASB - /Documents/{id}/Artifacts on" $which":  Test Failed " -f Red
exit 1
